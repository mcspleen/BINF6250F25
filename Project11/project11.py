...
class BaseHMM(object):
    ...
    Attributes:
        ...
        states (str): The hidden states within the HMM (default: None)
    ...
    __all__ = ['alphabet', 'states', 'emit_probs', 'trans_probs', 'init_probs']

    def __init__(self, alphabet="ACGT", states=None, init_probs=None,
    ...
    def _initialize_random(self, seed):
        ...
        if self.init_probs is None:
            ...
            states = self.states
            i_probs = nr.dirichlet(np.ones(len(self.states)))
        ...
        if self.trans_probs is None:
            ...
            # These aliases help keep line width down during the iteration and allows
            # for simple dictionary comprehension
            states = product(self.states, self.states)
            t_probs = np.nditer(nr.dirichlet(np.ones(len(self.states)), size=len(self.states)))
        ...
        if self.emit_probs is None:
            ...
            # These aliases help keep line width down during the iteration and allows
            # for simple dictionary comprehension
            states_letters = product(self.states, self.alphabet)
            e_probs = np.nditer(nr.dirichlet(np.ones(len(self.alphabet)), size=len(self.states)))
    ...
    def init_probs(self, init_probs):
        if init_probs is None:
            ...
            if len(init_probs) != len(self.states):
    ...
    def trans_probs(self, trans_probs):
        if trans_probs is None:
            ...
            if len(trans_probs) != len(self.states):
                raise ValueError('Transition probabilites must be a square matrix')
            if len(trans_probs[self.states[0]]) != len(self.states):
                raise ValueError('Transition probabilites must be a square matrix')
            if not np.allclose([sum(trans_probs[state].values()) for state in self.states], 1):
...
class HMM(BaseHMM):
    ...
    def forward(self, sequence):
        ...
        forward = {state: array('d', [0] * len(sequence)) for state in self.states}
        ...
        for seq_idx, next_state in product(range(1, len(sequence)), self.states):
            ...
            for curr_state in self.states:
                forward[next_state][seq_idx] += forward[curr_state][seq_idx - 1] * self.trans_probs[curr_state][
                    next_state]
        ...
        forward_prob = sum(forward[state][-1] for state in self.states)
    ...
    def backward(self, sequence):
        ...
        # Initialize the lattice. I am using an array here because it is more
        # performant in iteration and has a smaller footprint than mutliple dictionaries
        backward = {state: array('d', [0] * len(sequence)) for state in self.states}
        ...
        for seq_idx, last_state in product(rev_seq, self.states):

            # Since the forward algorithm just takes the sum across states from a given state, we need
            # to tease each of the states out
            # Furthermore, this is always using the data that has already been calculated from the right.
            for curr_state in self.states:
                backward[last_state][seq_idx - 1] += backward[curr_state][seq_idx] * self.trans_probs[last_state][
                    curr_state] * self.emit_probs[curr_state][sequence[seq_idx]]
        ...
        backward_prob = sum(
            backward[state][0] * self.init_probs[state] * self.emit_probs[state][sequence[0]] for state in self.states)
    ...
    def forward_backward(self, sequence):
        ...
        # Initialize the lattice. I am using an array here because it is more
        # performant in iteration and has a smaller footprint than mutliple dictionaries
        posterior = {state: array('d', [0] * len(sequence)) for state in self.states}
        ...
        for i, state in product(range(len(sequence)), self.states):
    ...
    def viterbi(self, sequence):
        ...
        previous = {state: np.log10(self.init_probs[state]) + np.log10(self.emit_probs[state][first_base]) for state in
                    self.states}
    ...
    def baum_welch(self, sequences, pseudocount=1e-100):
        ...
        def init_bw(pseudocount):
            ...
            """
            init = {state: pseudocount for state in self.states}
            trans = {state: {next_state: pseudocount for next_state in self.states} for state in self.states}
            emit = {state: {letter: pseudocount for letter in self.alphabet} for state in self.states}
        ...
        def scale_step(outer, init, trans, emit):
            ...
            emit_sum = {state: sum(emit[state].values()) for state in self.states}
            trans_sum = {state: sum(trans[state].values()) for state in self.states}
...
class ProfileHMM:
    ...
    def _build_states(self):
        """Create state topology: Begin, M1..ML, I0..IL, D1..DL, End."""
        states = ["Begin", "End"]
        states.append("I0")  # Insert before first match
    ...
    def _can_transition(self, from_state, to_state):
        ...
        # Handle End state
        if to_state == "End":
            return from_state in [f"M{self.L}", f"I{self.L}", f"D{self.L}"]

        # Don't allow transitions FROM End
        if from_state == "End":
            return False

        # Don't allow transitions TO Begin
        if to_state == "Begin":
            return False

        # Handle I0 (insert before first match)
        if from_state == "I0":
            return to_state in ["M1", "I0", "D1"]

        # Extract position numbers for other states
        try:
            from_type = from_state[0]
            from_pos = int(from_state[1:])

            to_type = to_state[0]
            to_pos = int(to_state[1:])
        except (ValueError, IndexError):
            return False

        # Standard profile HMM transitions
        if from_type == "M":
            return to_pos == from_pos + 1 and to_type in ["M", "I", "D"]
        elif from_type == "I":
            if from_pos == self.L:
                # Last insert can only go to End (handled above)
                return False
            return to_pos == from_pos + 1 and to_type in ["M", "I", "D"]
        elif from_type == "D":
            return to_pos == from_pos + 1 and to_type in ["M", "I", "D"]
    ...
    def _estimate_emissions_from_msa(self, msa, pseudocount):
        ...
        # Insert state emissions (background distribution)
        ...
        self.emit_probs["I0"] = bg_dist.copy()
        for i in range(1, self.L + 1):
            self.emit_probs[f"I{i}"] = bg_dist.copy()

        # Delete states are silent (no emissions)
        for i in range(1, self.L + 1):
    ...
    def _count_transitions_from_msa(self, msa):
        ...
        for seq in msa:
            ...
            for pos in range(self.L):
                if seq[pos] != '-':
                    path.append(f"M{pos + 1}")
                else:
                    path.append(f"D{pos + 1}")
            path.append("End")
    ...
    def _estimate_transitions_from_msa(self, msa, pseudocount):
        """Count state transitions from alignment to transition probabilities."""
        trans_counts = self._count_transitions_from_msa(msa)
        ...
        for state in self.states:
            ...
            for next_state in self.states:
                if self._can_transition(state, next_state):
                    count = trans_counts.get((state, next_state), 0) + pseudocount
                    outgoing[next_state] = count
        ...
        for i in range(1, n + 1):
            for state in self.states:
                ...
                for prev_state in self.states:
                    if prev_state in F[i - 1] and prev_state in self.trans_probs:
                        if state in self.trans_probs[prev_state]:
                            prev_logprob = F[i - 1][prev_state]
                            trans_logprob = log(self.trans_probs[prev_state][state])
                            log_sum = np.logaddexp(log_sum, prev_logprob + trans_logprob)

                if state.startswith('D'):
                    F[i][state] = log_sum  # Silent: no emission
                else:
                    emit_logprob = log(self.emit_probs.get(state, {}).get(sequence[i - 1], 1e-100))
                    F[i][state] = log_sum + emit_logprob

        total_logprob = -inf
        for s in self.states:
            if s in F[n] and s in self.trans_probs:
                if "End" in self.trans_probs[s]:
                    total_logprob = np.logaddexp(
                        total_logprob,
                        F[n][s] + log(self.trans_probs[s]["End"])
                    )
    ...
    def viterbi(self, sequence):
        ...
        for i in range(1, n + 1):
            for state in self.states:
                ...
                for prev_state in self.states:
                    if prev_state in V[i - 1] and prev_state in self.trans_probs:
                        if state in self.trans_probs[prev_state]:
        ...
        best_final_logprob = -inf
        best_state = None
        for s in V[n]:
            if s in self.trans_probs and "End" in self.trans_probs[s]:
                logprob, _ = V[n][s]
                final_logprob = logprob + log(self.trans_probs[s]["End"])
                if final_logprob > best_final_logprob:
                    best_final_logprob = final_logprob
                    best_state = s
        path = []
        current = best_state
    ...
    def _compute_posterior(self, forward, backward):
        ...
        n = len(forward) - 1
        posteriors = [{} for _ in range(n + 1)]

        for i in range(n + 1):
            log_total = -inf
            for state in self.states:
                if state in forward[i] and state in backward[i]:
                    log_total = np.logaddexp(log_total, forward[i][state] + backward[i][state])

            for state in self.states:
                if state in forward[i] and state in backward[i]:
                    posteriors[i][state] = exp(forward[i][state] + backward[i][state] - log_total)
                else:
                    posteriors[i][state] = 0.0
    ...
    def _log_likelihood(self, sequence):
        ...
        log_lik = -inf
        for state in self.states:
            if state in F[n] and state in self.trans_probs:
                if "End" in self.trans_probs[state]:
                    log_lik = np.logaddexp(
                        log_lik,
                        F[n][state] + log(self.trans_probs[state]["End"])
                    )
...
if __name__ == "__main__":
    hmm = ProfileHMM(alphabet="ACDEFGHIKLMNPQRSTVWY", length=5)
    msa = [
        "MVLVKFGKQI-",
        "MVLVK--GKQL",
        "MLVKFIPGKQI"
    ...
    for seq in msa:
        print(f"  {seq}")
    hmm.init_from_msa(msa)
    print(f"Number of states: {len(hmm.states)}")
    test_seq = "MVLVKFGKQI"
    print(f"\nTest sequence: {test_seq}")
    path = hmm.viterbi(test_seq)
    log_prob = hmm.forward(test_seq)
    print(f"\nViterbi path: {path}")
    print(f"Log probability: {log_prob:.4f}")
    print(f"Probability: {10 ** log_prob:.2e}")
